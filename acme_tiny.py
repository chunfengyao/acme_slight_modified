...
def get_crt(account_key, csr, acme_dir, log=LOGGER, CA=DEFAULT_CA, disable_check=False,
            directory_url=DEFAULT_DIRECTORY_URL, contact=None):
    ...
    def _do_request(url, data=None, err_msg="Error", depth=0):
        ...
        if depth < 100 and code == 400 and resp_data[
            ...
            #raise error
            sys.stderr.write(IndexError(resp_data)
                             .__str__())  # allow 100 retrys for bad nonces
            sys.exit(2)
        if code not in [200, 201, 204]:
            # raise error
            sys.stderr.write(ValueError(
                "{0}:\nUrl: {1}\nData: {2}\nResponse Code: {3}\nResponse: {4}".format(err_msg, url,
                                                                                      data, code,
                                                                                      resp_data))
                             .__str__())
            sys.exit(2)
    ...
    def _send_signed_request(url, payload, err_msg, depth=0):
        ...
        out = _cmd(["openssl", "dgst", "-sha256", "-sign", account_key], stdin=subprocess.PIPE,
    ...
    account, code, acct_headers = _send_signed_request(directory['newAccount'], reg_payload,
                                                       "Error registering")
    ...
    for auth_url in order['authorizations']:
        ...
        try:
            ...
            # raise error
            sys.stderr.write(ValueError(
                "Wrote file to {0}, but couldn't download {1}: {2}".format(wellknown_path,
                                                                           wellknown_url, e))
                             .__str__())
            sys.exit(3)

        # say the challenge is done
        _send_signed_request(challenge['url'], {},
        ...
        if authorization['status'] != "valid":
            # raise error
            sys.stderr.write(ValueError("Challenge did not pass for {0}: {1}".format(domain, authorization))
                             .__str__())
            sys.exit(4)
    ...
    order = _poll_until_not(order_headers['Location'], ["pending", "processing"],
                            "Error checking order status")
    if order['status'] != "valid":
        # raise error
        sys.stderr.write(ValueError("Order failed: {0}".format(order))
                         .__str__())
        sys.exit(5)

    # download the certificate
    certificate_pem, _, _ = _send_signed_request(order['certificate'], None,
...
def main(argv=None):
    ...
    parser.add_argument("--save-to", required=True,
                        help="path to the signed crt file to save")
    ...
    # save to file
    with open(args.save_to, "w") as text_file:
        text_file.write("{0}".format(signed_crt))
    # print to stdout
    sys.stdout.write(signed_crt)
    #normal exit code 0 for $?
    sys.exit(0)
